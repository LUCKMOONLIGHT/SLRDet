...
def delta2bbox(rois,
    ...
    da = denorm_deltas[:, 4::5] * 1.0 / np.pi * 180  # arc to angle
    ...
    if max_shape is not None:
        gx = gx.clamp(min=0, max=max_shape[0] - 1)
        gy = gy.clamp(min=0, max=max_shape[1] - 1)
        gw = gw.clamp(min=0, max=max_shape[0] - 1)
        gh = gh.clamp(min=0, max=max_shape[1] - 1)
...
def distance2bbox(points, distance, max_shape=None):
    """Decode distance prediction to bounding box. fcos_head
