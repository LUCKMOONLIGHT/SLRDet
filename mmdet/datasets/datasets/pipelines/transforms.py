...
class Resize(object):
    ...
    def _resize_bboxes(self, results):
        ...
        for key in results.get('bbox_fields', []):
            ...
            bboxes[:, 0:-1:2] = np.clip(bboxes[:, 0:-1:2], 0, img_shape[1] - 1)
            bboxes[:, 1:-1:2] = np.clip(bboxes[:, 1:-1:2], 0, img_shape[0] - 1)
            results[key] = bboxes
...
class RandomFlip(object):
    ...
    def __call__(self, results):
        ...
        if results['flip']:
            # flip image
            results['img'] = mmcv.imflip(results['img'])  #default:horizontal np.flip(img, axis=1)
            # flip bboxes
            for key in results.get('bbox_fields', []):
...
class RandomCrop(object):
    ...
    def __call__(self, results):
        ...
        if 'gt_bboxes' in results:
            ...
            valid_inds = (gt_bboxes[:, 2] > 0) & (gt_bboxes[:, 3] > 0)
        ...
        while True:
            ...
            for i in range(50):
                ...
                overlaps = bbox_overlaps(
                    patch.reshape(-1, 5), boxes.reshape(-1, 5)).reshape(-1)
