            ...
            if self.test_cfg.keep_all_stages:
                ...
                if self.with_mask:
                    ...
                    if det_bboxes.shape[0] == 0:
                        ...
                        det_bboxes[:, :4] = det_bboxes[:, :4] * scale_factor if rescale else det_bboxes
                        _bboxes = det_bboxes
        ...
        if self.with_mask:
            if det_bboxes.shape[0] == 0:
                ...
                if isinstance(scale_factor, float):  # aspect ratio fixed
                    ...
                    det_bboxes[:, :4] = det_bboxes[:, :4] * torch.from_numpy(scale_factor).to(det_bboxes.device) if rescale else det_bboxes
                    _bboxes = det_bboxes
        ...
        if self.with_mask:
            if det_bboxes.shape[0] == 0:
                ...
                for x, img_meta in zip(self.extract_feats(imgs), img_metas):
                    ...
                    scale_factor = img_meta[0]['scale_factor']
                    flip = img_meta[0]['flip']
                    _bboxes = bbox_mapping(det_bboxes[:, :5], img_shape,
                                           scale_factor, flip)
                    mask_rois = bbox2roi([_bboxes])
                    for i in range(self.num_stages):
                        mask_feats = self.mask_roi_extractor[i](
