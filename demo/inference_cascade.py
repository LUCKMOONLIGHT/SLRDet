...
def parse_args():
    ...
    parser.add_argument('--cropsize', help='patch image size', default=800, type=int)
    ...
    for imgfile in testImgList:
        ...
        if crop_num > 0:
            ...
            det_bboxes, det_labels = multiclass_nms(multi_bboxes, multi_scores,
                                                            cfg.test_cfg.rcnn.score_thr, cfg.test_cfg.rcnn.nms,
                                                            cfg.test_cfg.rcnn.max_per_img)

            if det_bboxes.shape[0] > 0:
                det_bboxes = np.array(det_bboxes)
                det_labels = np.array(det_labels) #0-14
                image = draw_boxes_with_label_and_scores(img, det_bboxes[:, :4], det_bboxes[:, 4], det_labels, 0)
                ...
                for sub_class in CLASS_DOTA:
                    if sub_class == 'back-ground':
                        continue
                    write_handle_r[sub_class] = open(os.path.join(args.saveTxtpath, '%s.txt' % sub_class), 'a+')
                ...
                for rect in det_bboxes[:, :4]:
                    boxes.append([rect[0], rect[2], rect[0], rect[3], rect[1],
