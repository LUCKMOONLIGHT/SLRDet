...
def draw_boxes_with_label_and_scores(img_array, boxes, scores, labels,  mode=0):
    ...
    for box, a_score, a_label in zip(boxes, scores, labels):
        draw_a_rectangel_in_img(draw_obj, box, a_score, a_label, color=STANDARD_COLORS2[a_label], width=2, mode=mode)
...
def draw_a_rectangel_in_img(draw_obj, box, score, label, color, width, mode):
    if mode == 0:
        ...
        draw_obj.line(xy=[top_right, bottom_right],
                      fill=color,
                      width=width)
        txt = LABEL_NAME_MAP[label + 1] + ':' + str(round(score, 2))
        ...
        draw_obj.line(xy=[(rect[3][0], rect[3][1]), (rect[0][0], rect[0][1])],
                      ...
                      width=width)
        txt = LABEL_NAME_MAP[label + 1] + ':' + str(round(score, 2))
        draw_obj.rectangle((rect[0][0], rect[0][1], (rect[0][0] + len(txt)*5+20), rect[0][1] - 10), fill=color)
        draw_obj.text(xy=(rect[0][0], rect[0][1] - 10),
                      text=txt,
                      fill='White',
                      font=FONT)
